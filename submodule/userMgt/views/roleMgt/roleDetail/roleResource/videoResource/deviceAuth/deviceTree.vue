<template>
	<div class="userMgt-roleMan-videoResource-deviceTree">
		<div class="top">
		...
		<div class="dev-body m-scroll">
				...
				<kd-tree
					...
					@add-data-from-server="addDataFromServer">
					 <span class="custom-tree-node" slot-scope="{ node, data }">
						  <kc-checkbox  v-if="!node.data.children  && node.data.leaf"  v-model="videoDeviceIds[node.data.id]" @change="()=> checkBoxSelected(node, data)">{{ node.label||node.data.deviceName }}</kc-checkbox>
			...
			<div class="search-table" v-show="!showTree">
				<kc-table :data="searchTableData" border highlight-current-row tooltip-effect="dark" style="width: 100%;cursor: pointer;" height="100%" @row-dblclick="checkBoxSelected">
					...
					<kc-table-column label="状态" width='100' prop="status">
						<template slot-scope="scope">
							<span >{{mapStatus(scope.row.status)}}</span></template>
					</kc-table-column>
				</kc-table>
			</div>
		</div>
...
<script>
	...
	export default {
		...
		computed: {
			...mapState({
				videoDeviceIds: state => state.roleMan.videoDeviceIds
			})
		},
		methods: {
			// 获取树的数据
			...
			checkBoxSelected(val) {
				let checkedFlag;
				if(val.data){
					if(this.videoDeviceIds[val.data.id]){
						checkedFlag = true;
					}else{
						checkedFlag = false;
					}
				}else{
					if(this.videoDeviceIds[val.id]){
						checkedFlag = false;
					}else{
						checkedFlag = true;
					}
				}
				this.$emit('addDeviceInfo', [{
